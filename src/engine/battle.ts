...
/** v5.69 · los vínculos del texto entre los conceptos de la sala que el jugador puede
 *  trazar ahora (tipos disponibles): la meta del mapa. */
export function vinculosDeLaSala(e: EstadoBatalla, ctx: ContextoBatalla): { from: string; to: string; tipo: string }[] {
...
export function pedirPista(e: EstadoBatalla, ctx: ContextoBatalla): string {
  ...
  const tipos = new Set(e.relacionesDisponibles)
  const mejor = ctx.contenido.aristas
...
export function compactarMapa(e: EstadoBatalla, ctx: ContextoBatalla): number {
  // componentes conexas de los armados por piezas compartidas
  ...
  while (restantes.length) {
    ...
    while (cambio) {
      ...
      for (let i = restantes.length - 1; i >= 0; i--) {
        if (restantes[i].piezas.some((u) => grupo.some((g) => g.piezas.includes(u)))) { grupo.push(restantes.splice(i, 1)[0]); cambio = true }
      }
    }
    grupos.push(grupo)
  }
  ...
  for (const grupo of grupos.filter((g) => g.length >= 3)) {
    ...
    if (!base) continue
    const pos = e.tablero.filter((x) => uids.includes(x.uid))
    const cx = pos.reduce((n, x) => n + x.x, 0) / Math.max(1, pos.length), cy = pos.reduce((n, x) => n + x.y, 0) / Math.max(1, pos.length)
    const constelacion: Pieza = { ...base, uid: `const:${eje}:${Date.now()}`, titulo: `${base.titulo} +${ids.length - 1}`, conceptIds: ids, cuerpo: `Constelación de ${ids.length} conceptos: ${ids.map((id) => ctx.contenido.conceptos[id]?.titulo ?? id).join(', ')}.` }
    e.mano = [...e.mano.filter((p) => !uids.includes(p.uid)), constelacion]
    e.tablero = [...e.tablero.filter((x) => !uids.includes(x.uid)), { uid: constelacion.uid, x: cx, y: cy }]
    for (const g of grupo) g.piezas = [constelacion.uid]
    compactados += 1
  }
  return compactados
}
...
export function afirmar(e: EstadoBatalla, ctx: ContextoBatalla): ResultadoTurno {
  ...
  const firma = (tool: string, ids: string[], param: string | null) => `${tool}|${[...ids].sort().join(',')}|${param ?? ''}`
  const yaEnMapa = new Set(e.mapa.trazos.map((x) => x.firma))
  // v5.68 · repetir un trazo que ya está en el mapa no suma: el mapa convierte la
  // repetición en un no-movimiento y empuja a explorar
  const repetidos = diag.veredictos.filter((v) => esAcierto(v.estado) && yaEnMapa.has(firma(v.trazo.tool, v.conceptIds, v.trazo.param)))
  for (const v of repetidos) {
    diag.fichas -= v.fichas; diag.mult -= v.mult
  ...
  // Todo se orienta a completar el mapa; la mano solo trae lo que aún suma.
  {
    const pendiente = new Set(vinculos.filter((a) => !parEnMapa(e, a.from, a.to)).flatMap((a) => [a.from, a.to]))
    const agotada = (p: Pieza) => !!p.conceptId && (p.clase === 'concepto' || p.clase === 'etiqueta' || p.clase === 'definicion') && !pendiente.has(p.conceptId) && e.fusionados.includes(p.conceptId)
    e.mazo = e.mazo.filter((p) => !agotada(p))
    e.descarte = e.descarte.filter((p) => !agotada(p))
  }
  // v5.68 · el mazo rota por novedad: primero las cartas cuyos conceptos aún tienen
  // vínculos por sostener en esta sala; las agotadas van al fondo
  {
    const enMapa2 = new Set(e.mapa.trazos.flatMap((x) => x.conceptIds))
    const usos = new Map<string, number>()
    for (const x of e.mapa.trazos) for (const id of x.conceptIds) usos.set(id, (usos.get(id) ?? 0) + 1)
    const novedad = (p: Pieza) => {
      if (!p.conceptId) return 1
      const incidentes = ctx.contenido.aristas.filter((a) => a.from === p.conceptId || a.to === p.conceptId).length
      return incidentes - (usos.get(p.conceptId) ?? 0) + (enMapa2.has(p.conceptId) ? 0.5 : 0)
    }
    e.mazo = [...e.mazo].sort((a, b) => novedad(b) - novedad(a))
  }
  ...
  for (const t of e.tablero) {
    ...
    if (!p) continue
    // v5.75 · todo lo sostenido se queda armado: en una identidad, nombre Y descripción, con su vínculo dibujado
    if (piezasArmadas.has(p.uid)) continue
...
export function siguienteTurno(e: EstadoBatalla, ctx?: ContextoBatalla): void {
  ...
  if (ctx && e.turnosSinAvance >= 2) {
    ...
    const jugable = (p: Pieza) => p.clase === 'concepto' || p.clase === 'etiqueta' || p.clase === 'definicion'
    const enManoIds = new Set(e.mano.filter((p) => jugable(p) && !e.tablero.some((x) => x.uid === p.uid)).map((p) => p.conceptId).filter((x): x is string => !!x))
    const tipos = new Set(e.relacionesDisponibles)
  ...
  e.pistaSuave = null
  if (ctx && e.turnosSinAvance === 1) {
    const dentro = new Set(e.conceptIdsCasilla)
    const armadosIds = [...new Set(e.armados.flatMap((x) => x.piezas).map((u) => e.mano.find((p) => p.uid === u)?.conceptId).filter((x): x is string => !!x))]
    const pendientesDe = (id: string) => ctx.contenido.aristas.filter((a) => (a.from === id || a.to === id) && dentro.has(a.from) && dentro.has(a.to) && !e.mapa.trazos.some((x) => x.conceptIds.includes(a.from) && x.conceptIds.includes(a.to))).length
    const mejor = armadosIds.sort((x, y) => pendientesDe(y) - pendientesDe(x))[0]
    if (mejor && pendientesDe(mejor) > 0) e.pistaSuave = mejor
  }
  e.turno += 1
